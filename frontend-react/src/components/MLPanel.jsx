...
// Black & White ML fraud typology classification + pre-confirmation flag widget

import { motion } from 'framer-motion'
import { Brain, Zap, AlertTriangle, CheckCircle2, Activity } from 'lucide-react'

const ANOMALY_CONFIG = {
  'HIGHLY ANOMALOUS': { color: '#ffffff', border: 'border-white', bg: 'bg-zinc-900', icon: AlertTriangle },
  'UNUSUAL':          { color: '#e4e4e7', border: 'border-zinc-500', bg: 'bg-zinc-950', icon: Activity },
  'TYPICAL':          { color: '#a1a1aa', border: 'border-zinc-700', bg: 'bg-zinc-950', icon: CheckCircle2 },
}
...
function ConfidenceBar({ label, value, index }) {
  return (
    ...
    >
      ...
      <span className="font-mono text-xs w-12 text-right flex-shrink-0 text-white font-medium">
        {value.toFixed(1)}%
      </span>
    </motion.div>
  )
...
function PatternBadge({ pattern }) {
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-zinc-900 border border-zinc-700 text-xs font-mono text-zinc-200 mr-2 mb-2">
...
function PreConfirmationFlag({ mlData }) {
  ...
  return (
    <motion.div
      ...
      className="mb-4 p-3.5 rounded-lg border border-white bg-zinc-900 flex items-start gap-3"
    >
      ...
      <div>
        ...
        <div className="text-xs text-zinc-300 mt-0.5">
          Predicted fraud probability: <span className="font-mono text-white font-semibold">{(mlData.pre_confirmation_confidence || 0).toFixed(1)}%</span> — 
...
export default function MLPanel({ mlData }) {
  ...
  return (
    <motion.div
      ...
      className="bg-zinc-950 border border-zinc-800 rounded-lg p-5 font-sans"
    >
      <div className="flex items-center justify-between pb-3 mb-4 border-b border-zinc-800/80">
        ...
        <span className="text-[10.5px] font-mono text-zinc-400 bg-zinc-900 border border-zinc-700 px-2 py-0.5 rounded-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        <div className="bg-black border border-zinc-800 rounded-lg p-3.5 text-center">
        ...
        <div className={`rounded-lg p-3.5 text-center border ${anomaly.border} ${anomaly.bg}`}>
          ...
          <div className="flex items-center justify-center gap-1.5 mt-1">
            <AnomalyIcon size={11} className="text-white" />
      ...
      {mlData.investigative_recommendation && (
        <div className="mt-4 border-t border-zinc-800/80 pt-4 text-xs text-zinc-300 leading-relaxed bg-black p-3 rounded-lg border border-zinc-800">
          <span className="text-white font-semibold">Investigative Guidance: </span>
          {mlData.investigative_recommendation}
        </div>
      )}
    </motion.div>
