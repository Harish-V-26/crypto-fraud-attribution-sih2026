...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Start real-time telemetry polling for 3D header
  startLiveTelemetry();
});

async function startLiveTelemetry() {
  async function update() {
    try {
      const [mRes, gRes] = await Promise.all([
        fetch(`${API}/api/crypto/market`).then(r => r.json()),
        fetch(`${API}/api/crypto/gas`).then(r => r.json())
      ]);
      const btc = mRes.assets?.BTC?.price_usd;
      const eth = mRes.assets?.ETH?.price_usd;
      const gas = gRes.ethereum?.standard || gRes.ethereum?.slow;

      const btcEl = document.getElementById('topBtcPrice');
      const ethEl = document.getElementById('topEthPrice');
      const gasEl = document.getElementById('topEthGas');

      if (btcEl && btc) btcEl.innerHTML = `BTC: <strong style="color:#6fd196;">$${btc.toLocaleString()}</strong>`;
      if (ethEl && eth) ethEl.innerHTML = `ETH: <strong style="color:#6fd196;">$${eth.toLocaleString()}</strong>`;
      if (gasEl && gas) gasEl.textContent = `GAS: ${gas} Gwei`;
    } catch (e) {
