...
// White background & Black text Risk Panel

import { motion } from 'framer-motion'
import { ShieldAlert, ShieldCheck, AlertTriangle, Building2, Link } from 'lucide-react'

const RISK_CONFIG = {
  LOW:      { badgeCls: 'badge-low',      label: 'Low Risk',      icon: ShieldCheck,  score_range: '0–29'  },
  MEDIUM:   { badgeCls: 'badge-medium',   label: 'Medium Risk',   icon: AlertTriangle, score_range: '30–59' },
  HIGH:     { badgeCls: 'badge-high',     label: 'High Risk',     icon: ShieldAlert,  score_range: '60–79' },
  CRITICAL: { badgeCls: 'badge-critical', label: 'Critical Risk', icon: ShieldAlert,  score_range: '80–100'},
}
  ...
  return (
    <div className="relative flex items-center justify-center w-36 h-36 mx-auto my-2">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <motion.span
          className="font-mono font-bold text-3xl text-black"
        ...
        <span className="text-[10px] text-zinc-500 font-mono tracking-wider uppercase font-semibold">/ 100 max</span>
      </div>
    </div>
  )
}
...
function RiskIndicators({ reasons }) {
  return (
    <ul className="space-y-2 mt-3">
      {reasons.length === 0 ? (
        <li className="text-xs text-zinc-500 font-mono">No elevated risk indicators detected for this address.</li>
      ) : reasons.map((r, i) => (
        <motion.li
          key={i}
          ...
          className="flex items-start gap-2.5 text-xs text-zinc-800 font-sans leading-relaxed"
        >
    ...
    >
          ...
          <div className="flex gap-1.5">
            {['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'].map(tier => (
              <div
                ...
                style={{
                  background: band === tier ? '#000000' : '#e4e4e7',
                }}
              />
            ))}
          </div>
          <div className="flex justify-between text-[10px] font-mono text-zinc-600 mt-1.5 px-0.5 uppercase tracking-wider font-semibold">
      ...
      <div className="bg-white border border-zinc-200 rounded-lg p-5 shadow-sm">
        <div className="flex items-center gap-2 mb-3 pb-2 border-b border-zinc-200">
          ...
          <h3 className="text-xs font-semibold uppercase tracking-wider text-black">Entity Attribution</h3>
        </div>
        {attribution ? (
          <div>
            <div className="text-xl font-bold text-black tracking-tight">{attribution.exchange}</div>
            <div className="font-mono text-xs text-zinc-600 mt-1 capitalize">{attribution.type}</div>
            {traceResult?.flags?.hops_to_exchange != null && (
              <div className="mt-2.5 inline-flex items-center gap-1.5 text-xs font-mono px-2.5 py-1 bg-zinc-100 rounded-md border border-zinc-300 text-black font-medium">
                <Link size={11} className="text-zinc-600" />
          ...
          <div>
            <div className="text-black font-semibold text-xs">No centralized exchange off-ramp identified within trace depth.</div>
      ...
      {/* Risk Indicators */}
      <div className="bg-white border border-zinc-200 rounded-lg p-5 shadow-sm">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-black pb-2 border-b border-zinc-200">
